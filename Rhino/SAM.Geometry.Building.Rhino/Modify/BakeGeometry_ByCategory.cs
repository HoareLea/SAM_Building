    ...
    public static partial class Modify
    {
        public static void BakeGeometry_ByCategory(this RhinoDoc rhinoDoc, IEnumerable<IPartition> partitions, bool cutOpenings = false, double tolerance = Core.Tolerance.Distance)
        {
            ...
            foreach (IPartition partition in partitions)
            {
                ...
                Layer layer = Core.Rhino.Modify.GetLayer(layerTable, layer_Host.Id, partition.GetType().ToString(), Building.Query.Color(partition));
                ...
                if (partition is IHostPartition)
                {
                    ...
                    foreach (IOpening opening in openings)
                    {
                        ...
                        layer = Core.Rhino.Modify.GetLayer(layerTable, layer_Opening.Id, opening.GetType().ToString(), Building.Query.Color(opening));
