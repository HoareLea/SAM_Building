...
namespace SAM.Core.Building
{
    public static partial class Query
    {
        ...
        {
            if (gasMaterial == null)
                return Building.DefaultGasType.Undefined;
        ...
        public static DefaultGasType DefaultGasType(params string[] values)
        {
            ...
            DefaultGasType result = Building.DefaultGasType.Undefined;
            foreach (string value in values_Temp)
            {
                result = Core.Query.Enum<DefaultGasType>(value);
                if (result != Building.DefaultGasType.Undefined)
                {
                    return result;
                }
            }
            ...
            foreach (DefaultGasType defaultGasType_Temp in Enum.GetValues(typeof(DefaultGasType)))
            {
                ...
                if (names_DefaultGasType.Count == 0 && defaultGasType_Temp == Building.DefaultGasType.SulfurHexaFluoride)
