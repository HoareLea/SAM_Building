    ...
    public static partial class Modify
    {
            ...
            foreach (HostPartitionType hostPartitionType in hostPartitionTypes_All)
            {
                ...
                if (duplicateHostPartitionTypes)
                {
                    ...
                    if (dictionary.Count == 1)
                    {
                        ...
                        HostPartitionType hostPartitionType_New = Core.Building.Create.HostPartitionType(Query.HostPartitionCategory(hostPartitionType), hostPartitionType.Name, materialLayers_Out);
                        foreach (IHostPartition hostPartition in hostPartitions)
                        {
                    ...
                    else
                    {
                        foreach (KeyValuePair<double, List<IHostPartition>> keyValuePair in dictionary)
                        {
                            ...
                            HostPartitionType hostPartitionType_New = Core.Building.Create.HostPartitionType(Query.HostPartitionCategory(hostPartitionType), name, materialLayers_Out);
        ...
        private static void UpdateMaterialsByHeatTransferCoefficients_OpeningTypes(this BuildingModel buildingModel, bool duplicateOpeningTypes, out List<OpeningType> openingTypes)
        {
            ...
            foreach (OpeningType openingType in openingTypes_All)
            {
                ...
                else
                {
                    ...
                    foreach (IOpening opening in openings)
                    {
                        double area_Aperture = opening.Area();

                        tilt += opening.Tilt() * System.Math.PI / 180 * area_Aperture;
                        area += area_Aperture;
                    }
        ...
        private static bool UpdateMaterialsByHeatTransferCoefficients(IEnumerable<Architectural.MaterialLayer> materialLayers_In, double tilt, BuildingModel buildingModel, out List<GasMaterial> gasMaterials, out List<Architectural.MaterialLayer> materialLayers_Out)
        {
            ...
            foreach (Architectural.MaterialLayer materialLayer in materialLayers_In)
            {
                ...
                DefaultGasType defaultGasType = Core.Building.Query.DefaultGasType(gasMaterial);
                ...
                GasMaterial gasMaterial_Default = Core.Building.Query.DefaultGasMaterial(defaultGasType);
                ...
                if (materialType == MaterialType.Opaque && defaultGasType == DefaultGasType.Air)
                    heatTransferCoefficient = Core.Building.Query.AirspaceConvectiveHeatTransferCoefficient(tilt, thickness);
                else
                    heatTransferCoefficient = Core.Building.Query.HeatTransferCoefficient(gasMaterial_Default, thickness, tilt);
                ...
                if (gasMaterial_New == null)
                {
                    ...
                    if (gasMaterial_New == null)
                    {
                        gasMaterial_New = Core.Building.Create.GasMaterial(gasMaterial_Default, name, name, name, thickness, heatTransferCoefficient);
