...
namespace SAM.Analytical
{
    public static partial class Query
    {
        public static IPartition Extend(this IPartition partition, IEnumerable<IPartition> partitions, double tolerance = Core.Tolerance.Distance)
        {
            ...
            foreach (IPartition partition_Temp in partitions)
            {
                Face3D face3D_Temp = partition_Temp?.Face3D;
                if (face3D_Temp == null)
                    continue;
            ...
            return Geometry.Building.Create.Partition(partition, partition.Guid, face3D, tolerance);
        }
